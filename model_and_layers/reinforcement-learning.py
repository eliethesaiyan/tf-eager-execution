...
for episode_id in range(num_episodes):
    ...
    for t in range(max_len_episodes):
        env.render()
        ...
        if len(replay_buffer) >= batch_size:
            ...
            q_value = model(tf.constant(batch_next_sate, dtype=tf.float32))
            y = batch_reward + (gamma * tf.reduce_max(q_value, axis=1)) * (1 - batch_done)

            
            with tf,GradientTape() as tape:
                loss = tf.losses.mean_squared_error(labels=y, prediction= tf.reduce_sum(model(tf.constant(batch_size)) *
